module aes(
    input [127:0] in,
    input [255:0] key,
    input state,           // Declare state as an input
    output reg [127:0] out // Declare out as a reg to hold output
);

wire [127:0] enc_out; // Intermediate wire for encryption output
wire [127:0] dec_out; // Intermediate wire for decryption output

// Instantiate encryption and decryption modules
AES_Encrypt #(256, 14, 8) encrypt (
    .in(in),
    .key(key),
    .out(enc_out)
);

AES_Decrypt #(256, 14, 8) decrypt (
    .in(in),
    .key(key),
    .out(dec_out)
);

// Use an always block to select the output based on state
always @* begin
    if (state == 0)
        out = enc_out;  // Output from encryption
    else
        out = dec_out;  // Output from decryption
end
...
// module aes_tb(enable, e128, d128, e192, d192, e256, d256);
